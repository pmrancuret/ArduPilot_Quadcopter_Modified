...
int16_t radio_out[]		= {1500,1500,1000,1500};			// PWM to ROLL PITCH Servos
float servo_out[] 	= {0,0,0,0};						// current values to the servos - -45 to 45 degrees, except [3] is 0 to 100
int16_t quadmot_out[]	= {1000,1000,1000,1000};			// four quad-motor commands.  Motors numbered in clockwise order, corresponding to channel number.  Elements of this array correspond to channel out when connected as quad copter.
...
union int_union intUnion = {0};
union long_union longUnion = {0};
...
void update_current_flight_mode(void)
{
	switch(control_mode){
		...
		case QUAD_SAFE:
			...
			servo_out[CH_THROTTLE]  = 10;		// even though servo_out is a float - this number should just hold the integer throttle stick command.  It is different from the roll, pitch, and yaw "servo" commands
